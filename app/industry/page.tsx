...
import { industryPages } from "@/app/lib/pseo";

export const metadata: Metadata = {
  title: "Industry ROAS Calculators",
  description: "Browse ROAS calculator landing pages by industry with benchmark context and profitability guidance.",
  alternates: {
    canonical: "https://roas-calculator.tech/industry",
  },
};
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 py-12 px-4 md:px-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-bold mt-2">Industry ROAS Calculators</h1>
          <p className="text-slate-600 mt-3">
            Select your vertical to use ROAS targets with industry-specific benchmark context.
          </p>
        </div>
        <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {industryPages.map((item) => (
            <Link
              key={item.slug}
              href={`/industry/${item.slug}/roas-calculator`}
